...
def draw_mean_plot_for_one_policy(results, algo, magnitudes, games, path, policy_id):
    ...
    for ii, mode in enumerate(modes):
        ...
        # mean_results, std_results = get_results_for_one_policy(
    ...
    plot_mean_results_with_bounds(all_results, left_std_results, right_std_results, magnitudes, f'{path}/plot_{algo}_{policy_id}.png')
...
# results have scheme: {"trained": , "random": }
# def draw_averaged_plots(results, magnitudes, algo, games, path_for_plots, policy_id):
#     draw_mean_plot_for_one_policy(results, algo, magnitudes, games, path_for_plots, policy_id)
...
def plot_for_other_algo():
    ...
    for algo in algos:
        ...
        for ii, algo in enumerate(algos):
            mean_results, min_results, max_results, std_results = get_results_for_one_policy(
                log_path=paths[ii], algo=algo, policy_id=policy_id, games=games, mode=modes[ii], max_noises=magnitudes)
...
if __name__ == "__main__":    
    magnitudes = ["0.01", "0.05", "0.1"] # [0.001, 0.005, 0.008, 0.01, 0.05, 0.1]
