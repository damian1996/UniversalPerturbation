...
def run_experiments_for_env(perturbation, env, random_act=False, algo=None, sticky_action_prob=0.0, meantime_test=False):
    batch_size = 128
    run_ids = [0,1,2,3]
    if algo is None:
        algo = 'rainbow'
    ...
    print_info = "Meantime test 1" if meantime_test else "Not seen run 1"
    print(print_info, results_final['ep_rewards'])
    ...
    results_final = generate_rollout.generate_clean_rollout(m, max_frames=2500, min_frames=2500,
            perturbation=perturbation, random_act=random_act, sticky_action_prob=sticky_action_prob)
    print_info = "Meantime test 2" if meantime_test else "Seen run 2"
    print(print_info, results_final['ep_rewards'])
    results_to_log.append(results_final['ep_rewards'])
    ...
    results_final = generate_rollout.generate_clean_rollout(m, max_frames=2500, min_frames=2500,
            perturbation=perturbation, random_act=random_act, sticky_action_prob=sticky_action_prob)
    print_info = "Meantime test 3" if meantime_test else "Seen run 3"
    print(print_info, results_final['ep_rewards'])
    results_to_log.append(results_final['ep_rewards'])
