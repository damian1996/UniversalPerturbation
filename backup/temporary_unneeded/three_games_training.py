...
def train_universal_perturbation_from_random_batches(models, to_be_perturbated=False, args=None, test_eps=1, max_frames=2500, 
                                                     min_frames=2500, dataset=None, nr_batches=0, max_noise=0., algo=None, lr=0.001,
                                                     data_loader=None, rep_buffer=None, all_training_cases=None,
                                                     output='', sticky_action_prob=0.0, 
    ...
    with tf.Graph().as_default() as graph, tf.Session(config=config) as sess:
        ...
        for idx, m in enumerate(ms):
            ...
            if idx == 0:
                ...
                T = import_model(m, X_t3, X_t3, scope=f"import5")
        ...
        train_op = tf.compat.v1.train.AdamOptimizer(lr).minimize(loss1 + loss2 + loss3)
        ...
        while not training_finished:
            ...
            _, l1, l2, l3 = sess.run([train_op, loss1, loss2, loss3], feed_dict=train_dict)
            losses1.append(l1)
            losses2.append(l2)
            losses3.append(l3)
