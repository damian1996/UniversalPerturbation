...
def train_universal_perturbation_from_random_batches(models, to_be_perturbated=False, args=None, test_eps=1, max_frames=2500, 
    ...
    with tf.Graph().as_default() as graph, tf.Session(config=config) as sess:
        ...
        obss3 = tf.placeholder(tf.float32, [None] + list(sh), name=f"obss3")
        obss4 = tf.placeholder(tf.float32, [None] + list(sh), name=f"obss4")

        clean_actt1 = tf.placeholder(tf.int32, [None], name=f"action1")
        clean_actt2 = tf.placeholder(tf.int32, [None], name=f"action2")
        clean_actt3 = tf.placeholder(tf.int32, [None], name=f"action3")
        clean_actt4 = tf.placeholder(tf.int32, [None], name=f"action4")

        X_t1 = obss1 + universal_perturbation
        X_t2 = obss2 + universal_perturbation
        X_t3 = obss3 + universal_perturbation
        X_t4 = obss4 + universal_perturbation
        ...
        for idx, m in enumerate(ms):
            ...
            if idx == 0:
                ...
                T = import_model(m, X_t3, X_t3, scope=f"import3")
            else:
                T = import_model(m, X_t4, X_t4, scope=f"import4")
            ...
            if idx == 0:
                ...
                loss3 = -tf.losses.sparse_softmax_cross_entropy(labels=clean_actt3, logits=policy)
            else:
                loss4 = -tf.losses.sparse_softmax_cross_entropy(labels=clean_actt4, logits=policy)

        train_op = tf.compat.v1.train.AdamOptimizer(lr).minimize(loss1 + loss2 + loss3 + loss4)
        ...
        while not training_finished: 
            ...
            train_dict = {
                ...
                X_t3: arr_pert_obs[2], clean_actt3: np.array(arr_act[2]), obss3: arr_obs[2],
                X_t4: arr_pert_obs[3], clean_actt4: np.array(arr_act[3]), obss4: arr_obs[3]
            }

            _, l1, l2, l3, l4 = sess.run([train_op, loss1, loss2, loss3, loss4], feed_dict=train_dict)
            losses1.append(l1)
            losses2.append(l2)
            losses3.append(l3)
            losses4.append(l4)
            ...
            if frame_count % rep_buffer.replay_after_batches == 0:
                ...
                dataset, training_finished = data_loader.get_updated_buffer(it)
                it += 1
